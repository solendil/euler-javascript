...
problem[54] = {
    ...
    solver: function() {
      ...
      function evaluate(hand) {
        ...
        function rank(card) {return rankString.indexOf(card[0]);}
        function rankHexa(card) {return rankString.indexOf(card[0]).toString(16);}
        let rankh = new CategorySorter();
        let suith = new CategorySorter();
        for (let c of hand) {
          rankh.add(rank(c),c);
          suith.add(c[1],c);
        }
        let rankCounts = rankh.counts().join('');
        let rankHisto = rankh.histogram();
        // SCORE with first digit for global ranks and subsequent digits for equalities
        let value = "000000".split('');
        if (rankCounts==="41") {
          value[0] = "8";
          // console.log("Four of a Kind");
        } else if (rankCounts==="32") {
          value[0] = "7";
          // console.log("Full House");
        } else if (rankCounts==="311") {
          value[0] = "4";
          // console.log("Three of a Kind");
        } else if (rankCounts==="221") {
          value[0] = "3";
          // console.log("Two Pairs");
        } else if (rankCounts==="2111") {
          value[0] = "2";
          value[1] = rankHexa(rankHisto[0].values[0]);
          value[2] = rankHexa(rankHisto[1].values[0]);
          value[3] = rankHexa(rankHisto[2].values[0]);
          value[4] = rankHexa(rankHisto[3].values[0]);
          // console.log("One Pair");
        } else {
          var flush = suith.counts().join('')==="5";
          hand.sort((a,b) => rank(a)-rank(b));
          var range = rank(hand[hand.length-1])-rank(hand[0]);
          var straight = (range==4);
          var royal = rank(hand[0])===rank('T');
          if (straight) {
            if(flush) {
              if (royal) {
                value[0] = "A";
      ...
      for (let hand of this.hands) {
        let hand1 = hand.trim().split(' ').slice(0,5);
        let hand2 = hand.trim().split(' ').slice(5,10);
        let score1 = evaluate(hand1);
        let score2 = evaluate(hand2);
        if (score1===score2) {
          console.log("!!!! INDECIDABLE");
          console.log(hand);
          console.log("    ",score1.toString(16), score2.toString(16));
          tie++;
        } else {
          if (score1>score2)
            p1++;
